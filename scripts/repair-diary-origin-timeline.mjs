...
import {
  ...
  MAX_DIARY_ORIGIN_REPAIR_LIMIT
} from "./diary-origin-timeline-repair.mjs";
...
const APPLY_CONFIRMATION = "diary-origin-timeline";
const WRANGLER_ENTRY = path.join(process.cwd(), "node_modules", "wrangler", "bin", "wrangler.js");

export function usage() {
  return `Usage:
  npm run repair:diary-origin-timeline -- --remote [options]
...
Options:
  ...
  --limit <n>           Maximum invalid origins in this run. Default: ${DEFAULT_LIMIT}; max: ${MAX_DIARY_ORIGIN_REPAIR_LIMIT}
  --apply               Delete one bounded set of invalid-origin memberships and owned in_episode relations.
  --confirm <text>      Required with --apply; must equal ${APPLY_CONFIRMATION}
  --json                Emit machine-readable JSON.
  --help                Show this help.

Without --apply this command executes SELECT only. It never deletes free-text relations
or rebuilds diary timeline sequences, and it does not loop automatically.`;
}

export function parseDiaryOriginRepairArgs(argv) {
  const args = {
    db: DEFAULT_DB,
  ...
  if (!Number.isInteger(args.limit) || args.limit < 1 || args.limit > MAX_DIARY_ORIGIN_REPAIR_LIMIT) {
    throw new Error(`--limit must be an integer between 1 and ${MAX_DIARY_ORIGIN_REPAIR_LIMIT}.`);
  }
...
function executeQuery(args, query) {
  const result = spawnSync(process.execPath, [
    ...
    env: {
      ...process.env,
      WRANGLER_LOG_PATH: path.join(process.cwd(), ".wrangler", "diary-origin-timeline-repair.log")
    },
    stdio: ["ignore", "pipe", "pipe"]
  });
  if (result.status !== 0) {
    throw new Error(
      `diary origin timeline repair query failed (${query.name}):\n`
...
export function runDiaryOriginRepair(args, execute = executeQuery) {
  const dryRunQuery = buildDiaryOriginRepairDryRunQuery(args);
  assertReadOnlyDiaryOriginRepairQuery(dryRunQuery);
  if (!args.apply) {
    const preview = execute(args, dryRunQuery);
    if (preview.rowsWritten !== 0) throw new Error("diary_origin_repair_dry_run_reported_rows_written");
  ...
  let changed = 0;
  for (const query of buildDiaryOriginRepairApplyQueries(args)) {
    changed += execute(args, query).changes;
  }
  ...
  return {
    ...
    limit: args.limit,
    changed,
    remaining: Number(after.repairable_origins ?? 0),
    has_more: Boolean(Number(after.has_more ?? 0))
  };
}

function printHumanReport(report) {
  console.log(`Diary origin timeline repair: ${report.mode}`);
  console.log(`Namespace: ${report.namespace}`);
  console.log(JSON.stringify(report, null, 2));
}

async function main() {
  const args = parseDiaryOriginRepairArgs(process.argv.slice(2));
  if (args.help) {
    console.log(usage());
    return;
  }
  const report = runDiaryOriginRepair(args);
