...
const checks = [
  ...
  ["Safety: coordinate proposals are batched and exceptions are reviewable", files.debug.includes("BACKFILL_MODEL_BATCH_SIZE = 5") && files.debug.includes("splitCoordinatePatch") && files.debug.includes('mode: apply ? "auto_apply_with_exception_review" : "dry_run"')],
