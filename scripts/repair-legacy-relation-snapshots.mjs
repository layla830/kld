...
const APPLY_CONFIRMATION = "legacy-relation-snapshots";
const WRANGLER_ENTRY = path.join(process.cwd(), "node_modules", "wrangler", "bin", "wrangler.js");

export function usage() {
  return `Usage:
  npm run repair:legacy-relation-snapshots -- --remote [options]
...
Options:
  ...
  --limit <n>           Maximum rows in this run. Default: ${DEFAULT_LIMIT}; max: ${MAX_LEGACY_RELATION_SNAPSHOT_REPAIR_LIMIT}
  --apply               Execute one bounded normalization statement.
  --confirm <text>      Required with --apply; must equal ${APPLY_CONFIRMATION}
  --json                Emit machine-readable JSON.
  --help                Show this help.

Without --apply this command executes SELECT only. Apply only adds the proven
legacy relation_was_present=true field; it does not loop automatically.`;
}

export function parseLegacyRelationSnapshotRepairArgs(argv) {
  const args = {
    db: DEFAULT_DB,
  ...
  if (
    ...
    || args.limit < 1
    || args.limit > MAX_LEGACY_RELATION_SNAPSHOT_REPAIR_LIMIT
  ) {
    throw new Error(
      `--limit must be an integer between 1 and ${MAX_LEGACY_RELATION_SNAPSHOT_REPAIR_LIMIT}.`
    );
  }
...
function executeQuery(args, query) {
  const result = spawnSync(process.execPath, [
    ...
    env: {
      ...
      WRANGLER_LOG_PATH: path.join(
        process.cwd(),
        ".wrangler",
        "legacy-relation-snapshot-repair.log"
      )
    },
  ...
  if (result.status !== 0) {
    throw new Error(
      `legacy relation snapshot repair query failed (${query.name}):\n`
...
export function runLegacyRelationSnapshotRepair(args, execute = executeQuery) {
  const dryRunQuery = buildLegacyRelationSnapshotDryRunQuery(args);
  assertReadOnlyLegacyRelationSnapshotQuery(dryRunQuery);
  if (!args.apply) {
    const preview = execute(args, dryRunQuery);
    if (preview.rowsWritten !== 0) {
      throw new Error("legacy_relation_snapshot_dry_run_reported_rows_written");
    }
  ...
  const applied = execute(args, buildLegacyRelationSnapshotApplyQuery(args));
...
function printHumanReport(report) {
  console.log(`Legacy relation snapshot repair: ${report.mode}`);
  console.log(`Namespace: ${report.namespace}`);
  console.log(JSON.stringify(report, null, 2));
}

async function main() {
  const args = parseLegacyRelationSnapshotRepairArgs(process.argv.slice(2));
  if (args.help) {
    console.log(usage());
    return;
  }
