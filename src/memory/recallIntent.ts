...
const DURABLE_QUESTION_RE = /(?:平时|一直|总是|以后|应该|喜欢|不喜欢|偏好|习惯|规则|底线|约定|承诺).{0,24}(?:什么|哪|谁|为什么|怎么|是否|是不是)|(?:是谁|谁来|谁负责).{0,16}(?:提|说|做|处理|决定)/;
...
export function analyzeRecallNeed(prompt: string): RecallAnalysis {
  ...
  const hintedFactKeys = factKeysForQueryHint(query);
  const durableQuestion = DURABLE_QUESTION_RE.test(query);
  const recallSignal = MEMORY_VERB_RE.test(query) || TIME_WORD_RE.test(query) || DATE_RE.test(query) || NATURAL_RECALL_RE.test(query) || hintedFactKeys.length > 0 || durableQuestion;
  ...
  const reasons = [
    recallSignal ? "explicit_recall_signal" : "",
    hintedFactKeys.length > 0 ? "query_hint" : "",
    durableQuestion ? "durable_question" : "",
    recallSignal && contextHint ? "context_hint" : ""
  ].filter(Boolean);
