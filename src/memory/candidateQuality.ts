...
function sourceChunkCount(candidate: MemoryCandidateRecord): number {
  try {
    const ids = JSON.parse(candidate.source_chunk_ids_json) as unknown;
...
export function assessCandidateQuality(candidate: MemoryCandidateRecord): CandidateQualityAssessment {
  ...
  if (sourceChunkCount(candidate) > 1) {
