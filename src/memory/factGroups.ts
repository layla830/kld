...
export async function proposeFactGroups(env: Env, namespace: string, apply = false): Promise<{ scanned: number; proposed: number; queued: number; groups: FactGroupProposal[] }> {
  const rows = await env.DB.prepare(`SELECT * FROM memories WHERE namespace = ? AND status = 'active' AND (fact_key IS NULL OR fact_key = '') AND thread IS NOT NULL AND importance >= 0.6 AND type IN ('rule','lesson','core','preference','identity','note') ORDER BY thread, importance DESC, updated_at DESC LIMIT 20`).bind(namespace).all<MemoryRecord>();
  const memories = rows.results ?? [];
  if (memories.length < 2) return { scanned: memories.length, proposed: 0, queued: 0, groups: [] };
  ...
  return { scanned: memories.length, proposed: groups.length, queued: apply ? groups.length : 0, groups };
}
