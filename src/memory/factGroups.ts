...
export async function proposeFactGroups(env: Env, namespace: string, apply = false, threadOffset = 0): Promise<{ thread: string | null; threadOffset:number; scanned: number; proposed: number; queued: number; groups: FactGroupProposal[] }> {
  const offset = Math.min(Math.max(Math.floor(threadOffset), 0), 1000);
  const threadRow = await env.DB.prepare(`SELECT thread, COUNT(*) AS count FROM memories WHERE namespace = ? AND status = 'active' AND (fact_key IS NULL OR fact_key = '') AND thread IS NOT NULL AND importance >= 0.6 AND type IN ('rule','lesson','core','preference','identity','note') GROUP BY thread HAVING count >= 2 ORDER BY count DESC, thread LIMIT 1 OFFSET ?`).bind(namespace, offset).first<{thread:string;count:number}>();
  if (!threadRow?.thread) return { thread:null, threadOffset:offset, scanned:0, proposed:0, queued:0, groups:[] };
  ...
  if (memories.length < 2) return { thread:threadRow.thread, threadOffset:offset, scanned: memories.length, proposed: 0, queued: 0, groups: [] };
  ...
  return { thread:threadRow.thread, threadOffset:offset, scanned: memories.length, proposed: groups.length, queued: apply ? groups.length : 0, groups };
}
