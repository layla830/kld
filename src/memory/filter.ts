...
function extractJsonArrayFromString(text: string): unknown[] | null {
  ...
  try {
    ...
    // Try extracting JSON from providers that wrap the answer in prose.
  }
...
function buildPrompt(input: {
  ...
  return [
    ...
    "",
    "类型提示：",
    "- type=timeline_day 是某一天的事件目录。用户问某天/那天/日期发生了什么时，优先保留 timeline_day；diary 只在没有 timeline_day 或用户要完整细节时作为补充。",
    "- type=milestone、fact、preference、profile 是稳定事实。用户问'是什么'、'怎么来的'、'由来'、'定义'时，优先保留这类直接命中的候选。",
    "- type=quote 是原话或原始场景。只有它直接命中用户问的名词、事件、时间或口令时才保留；不要因为同一天出现就保留无关 quote。",
    "- type=diary 是原始长文本兜底。不要只因为它更完整就压过直接命中的 timeline_day、milestone、fact 或 quote。",
    "- handoff、交接、启动上下文类候选默认丢弃；只有用户明确问交接/handoff 时才保留。",
    "",
    "规则：",
  ...
  try {
    ...
    return {
      ...
      meta: {
        ...
        reason: filtered.length > 0 ? undefined : "model_returned_empty",
        output_shape: describeModelOutput(output)
      }
    };
  } catch (error) {
