...
async function fetchStats(env: Env): Promise<{ active: number; deleted: number; total: number; vectorized: number }> {
  ...
  const vectorized = await env.DB
    .prepare("SELECT COUNT(*) AS count FROM memories WHERE vector_id IS NOT NULL AND vector_id != ''")
    .first<{ count: number }>();
  const map = new Map((rows.results ?? []).map((row) => [row.status, row.count]));
  const active = map.get("active") ?? 0;
  const deleted = map.get("deleted") ?? 0;
  return { active, deleted, total: [...map.values()].reduce((sum, count) => sum + count, 0), vectorized: vectorized?.count ?? 0 };
}
...
</style></head><body><div class="page"><header><div class="heart">♡</div><h1>我们的记忆小家</h1><div class="subtitle">MEMORY HOME</div></header>${renderTabs(input)}${dashboard}${composer}<div class="header-row"><span class="section-title">${listTitle}</span><div class="divider"></div><a class="small-btn" href="${qs(input, { page: 1 })}">刷新</a></div>${list}${renderPagination(input, data.total)}</div></body></html>`;
}
...
export async function handleAdminMemories(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  const [stats, types, memories, heatmap, startup] = await Promise.all([
    fetchStats(env),
    fetchTypes(env),
