...
export function renderTimelineCandidate(candidate: MemoryCandidateRecord): string {
  ...
  const approve = available
    ? `<form method="POST" action="/admin/memories/x-timeline/approve" onsubmit="return confirm('只修正日期和 timeline 标签，正文与主题线都不会改变。确认吗？')"><input type="hidden" name="id" value="${attr(candidate.id)}">${dateControl}<button class="action-btn approve-review">${repair ? "确认修复日期" : "确认补日期"}</button></form>`
