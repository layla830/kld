...
function renderArchiveCandidate(candidate: MemoryCandidateRecord, payload: Record<string, unknown>, before: Record<string, unknown>, approved: boolean): string {
  ...
  const actionButtons = approved
    ? `<form method="POST" action="${ADMIN_BOARD_ROUTES.rollbackOperationalReview.path}" onsubmit="return confirm('按快照恢复这次 M 轴操作？')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">回滚这次操作</button></form>`
    : `<form method="POST" action="${ADMIN_BOARD_ROUTES.approveOperationalReview.path}" onsubmit="return confirm('${attr(effect)}')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">确认归档</button></form><form method="POST" action="${ADMIN_BOARD_ROUTES.rejectOperationalReview.path}" onsubmit="return confirm('保留这条记忆，不做归档？')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn delete">保留，不归档</button></form>`;
...
function renderRelationCandidate(candidate: MemoryCandidateRecord, payload: Record<string, unknown>, before: Record<string, unknown>, approved: boolean): string {
  ...
  const batchSelector = candidate.status === "pending"
    ...
    : "";
  const actionButtons = approved
    ? `<form method="POST" action="${ADMIN_BOARD_ROUTES.rollbackOperationalReview.path}" onsubmit="return confirm('按快照恢复这条关系边？')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">恢复这条边</button></form>`
    : `<form method="POST" action="${ADMIN_BOARD_ROUTES.approveOperationalReview.path}" onsubmit="return confirm('${attr(effect)}')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">只删这条边</button></form><form method="POST" action="${ADMIN_BOARD_ROUTES.rejectOperationalReview.path}" onsubmit="return confirm('保留这条关系边？不会修改两端记忆。')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn delete">保留这条边</button></form>`;
...
function renderRelationReviewCandidate(
  ...
  const actionButtons = approved
    ? `<form method="POST" action="${ADMIN_BOARD_ROUTES.rollbackOperationalReview.path}" onsubmit="return confirm('撤销这次关系批准？只会移除本次新建的边。')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">回滚这次建边</button></form>`
    : `<form method="POST" action="${ADMIN_BOARD_ROUTES.approveOperationalReview.path}" onsubmit="return confirm('${attr(effect)}')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">批准建立关系</button></form><form method="POST" action="${ADMIN_BOARD_ROUTES.rejectOperationalReview.path}" onsubmit="return confirm('拒绝这条关系建议？不会修改任何记忆。')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn delete">拒绝关系</button></form>`;
...
function renderFactTransitionCandidate(candidate: MemoryCandidateRecord, payload: Record<string, unknown>, approved: boolean): string {
  ...
  const actionButtons = approved
    ? `<form method="POST" action="${ADMIN_BOARD_ROUTES.rollbackOperationalReview.path}" onsubmit="return confirm('恢复被取代的旧事实，让它重新参与召回？')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">回滚这次取代</button></form>`
    : `<form method="POST" action="${ADMIN_BOARD_ROUTES.approveOperationalReview.path}" onsubmit="return confirm('${attr(effect)}')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn approve-review">确认取代旧事实</button></form><form method="POST" action="${ADMIN_BOARD_ROUTES.rejectOperationalReview.path}" onsubmit="return confirm('保留两条 active 事实，不执行取代？')"><input type="hidden" name="id" value="${attr(candidate.id)}"><button class="action-btn delete">两条都保留</button></form>`;
