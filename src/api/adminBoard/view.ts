...
function renderComposer(input: PageInput, typeOptions = ""): string {
  if (input.tab === "message") {
    return `<section class="card"><form method="POST" action="${ADMIN_BOARD_ROUTES.create.path}"><input type="hidden" name="kind" value="message"><textarea name="content" placeholder="给小克留言..."></textarea><div class="footer"><span class="char-count">0</span><button class="btn" type="submit">发送</button></div></form></section>`;
  }
  if (input.tab === "diary") {
    return `<section class="card"><form method="POST" action="${ADMIN_BOARD_ROUTES.create.path}"><div class="filters"><select name="author" class="filter-select"><option value="layla">Layla</option><option value="kld">KLD</option></select><select name="mood" class="filter-select">${renderMoodOptions("", "心情")}</select></div><input type="hidden" name="kind" value="diary"><textarea name="content" placeholder="写下今天的日记..."></textarea><div class="footer"><span class="char-count">0</span><button class="btn" type="submit">保存</button></div></form></section>`;
  }
  if (input.tab === "quote") {
    return `<section class="card"><form method="POST" action="${ADMIN_BOARD_ROUTES.create.path}"><input type="hidden" name="kind" value="quote"><div class="input-group"><div class="input-label">语录内容</div><textarea name="content" placeholder="粘贴或输入语录..."></textarea></div><div class="input-group"><div class="input-label">分类</div><input type="text" name="category" placeholder="例如: 关于爱 / 哲学 / 让我哭的 / 骚话"></div><div class="footer"><span class="char-count">0</span><button class="btn" type="submit">保存</button></div></form></section>`;
  }
...
function renderMemory(record: MemoryRecord, tab: string): string {
  ...
  const deleteForm = record.status === "active" ? `<form method="POST" action="${ADMIN_BOARD_ROUTES.delete.path}" class="delete-form" onsubmit="return confirm('确认删除吗？这会软删除，不会立刻物理清空。')"><input type="hidden" name="id" value="${attr(record.id)}"><button class="action-btn delete" type="submit">删除</button></form>` : "";
...
export function renderPage(input: PageInput, data: PageData): string {
  ...
  const timelineReviewGuide = renderTimelineReviewGuide({ show: input.tab === "x-review", pending: data.total, status: data.timelineBackfill });
  const metabolismReviewGuide = renderMetabolismReviewGuide(input.tab === "m-review", data.operationalPending);
