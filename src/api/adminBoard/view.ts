...
interface PageData {
  ...
  resolvedCandidates: MemoryCandidateRecord[];
  heatmap: HeatDay[];
...
function renderFiveAxisDeadLetters(data: Lmc5DashboardData): string {
  if (data.deadLetters.length === 0) return "";
  const rows = data.deadLetters.map((item) => `<div class="lmc-duplicate"><div><strong>${htmlEscape(item.memory_id)}</strong><div class="char-count">revision ${item.memory_revision ?? 1} · 尝试 ${item.attempts} 次 · ${htmlEscape(item.last_error || "未知错误")}</div></div><form method="POST" action="/admin/memories/lmc5/retry-dead-letter"><input type="hidden" name="id" value="${attr(item.id)}"><button class="action-btn approve-review">重新入队</button></form></div>`).join("");
  return `<section class="card lmc-panel"><div class="header-row"><span class="section-title">五维死信</span><div class="divider"></div><span class="score-pill">${data.deadLetters.length} 条</span></div><div class="review-warning">这些任务已连续失败 5 次并停止自动重试。先看最后错误，再按条重新入队；重试会同时释放该 revision 已耗尽的轴。</div><div class="lmc-duplicates">${rows}</div></section>`;
}
...
function renderLmc5Dashboard(data: Lmc5DashboardData | null): string {
  ...
  return [
    ...
    renderFiveAxisDeadLetters(data),
