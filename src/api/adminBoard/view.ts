...
export function renderPage(input: PageInput, data: PageData): string {
  ...
  const metabolismReviewGuide = input.tab === "m-review" ? `<section class="card lmc-panel"><div class="header-row"><span class="section-title">Z 事实状态 · Y 关系清理 · M 安全巡检</span><div class="divider"></div><span class="score-pill">${data.operationalPending} 条待审</span></div><div class="lmc-explain"><p><strong>Z 负责事实状态：</strong>同一 fact_key 有多个 active 版本时，逐条建议保留最佳事实、取代较弱旧事实；批准前会重新核对两条记忆和当前排名。</p><p><strong>Y/M 负责关系与代谢：</strong>安全关系自动建边；这里只人工处理过期项目、自环、悬空边和重复对称边。</p><p><strong>统一审核：</strong>Z/M 共用同一套卡片骨架、批准、拒绝和回滚端点；Z 不支持批量取代。</p></div><form method="POST" action="/admin/memories/m-review/scan"><button class="btn" type="submit">重新扫描 Z/M 候选</button></form></section>` : "";
