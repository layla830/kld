...
export function renderPage(input: PageInput, data: PageData): string {
  ...
  const factBatchBar = input.tab === "review" && data.candidates.some((candidate) => candidate.action === "diary_split_fact" && candidate.status === "pending")
    ...
    : "";
  const candidateList = input.tab === "review" ? qualityBatchBar + factBatchBar + data.candidates.map(renderMemoryCandidate).join("") : input.tab === "x-review" ? data.candidates.map(renderTimelineCandidate).join("") : input.tab === "m-review" ? data.candidates.map(renderMetabolismCandidate).join("") : "";
