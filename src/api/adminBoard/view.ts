...
export function renderPage(input: PageInput, data: PageData): string {
  ...
  const diarySplitPreview = input.tab === "timeline" ? `<section class="card lmc-panel"><div class="header-row"><span class="section-title">日记拆分 v2</span><div class="divider"></div><span class="score-pill">只读</span></div><div class="lmc-explain"><p><strong>试拆最近 3 篇：</strong>由 Worker 直接读取 D1 并调用小模型，只展示通过原文证据校验的草稿。</p><p><strong>不会发生：</strong>不会创建记忆、审核候选、关系边或向量。</p></div><form method="POST" action="/admin/memories/diary-split/preview"><button class="btn" type="submit">只读试拆最近 3 篇</button></form></section>` : "";
