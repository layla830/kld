...
export function renderPage(input: PageInput, data: PageData): string {
  ...
  const listTitle = input.tab === "message" ? "历史留言" : input.tab === "diary" ? "我们的日记" : input.tab === "quote" ? "我的语录" : input.tab === "timeline" ? "分段日记" : input.tab === "review" ? "Dream 审核" : input.tab === "x-review" ? "X 时间轴审核" : input.tab === "m-review" ? "M 代谢审核" : input.date ? `${input.date} 的记忆` : input.q ? `${searchPrefix}：${input.q}` : "记忆列表";
  ...
  const metabolismReviewGuide = input.tab === "m-review" ? `<section class="card lmc-panel"><div class="header-row"><span class="section-title">安全代谢巡检</span><div class="divider"></div><span class="score-pill">${data.metabolismPending} 条待审</span></div><div class="lmc-explain"><p>这里只处理过期 project_state 和异常关系边。身份、关系时刻、日记、正文合并都不在本轮范围内。每次批准前保存完整快照，批准后可回滚。</p></div><form method="POST" action="/admin/memories/m-review/scan"><button class="btn" type="submit">扫描现有记忆</button></form></section>` : "";
