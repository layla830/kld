...
export function renderMemoryCandidate(candidate: MemoryCandidateRecord): string {
  ...
  const batchChoice = candidate.action === "diary_split_fact" && candidate.status === "pending"
    ...
    : "";
  const candidateBatchChoice = `<label class="m-batch-choice"><input class="candidate-batch-checkbox" form="candidate-batch-form" type="checkbox" name="ids" value="${attr(candidate.id)}" onchange="updateCandidateBatch()"> 选入批量同意/拒绝</label>`;
