...
export async function handleAdminRebuildEmbeddings(request: Request, env: Env): Promise<Response> {
  ...
  const details = records.map((record, index) => {
    ...
    if (result.status === "fulfilled") return { id: record.id, ok: result.value };
    return { id: record.id, ok: false, error: result.reason instanceof Error ? result.reason.message : String(result.reason) };
  });
