...
async function saveProgress(env: Env, request: Request): Promise<Response> {
  ...
  ).bind(bookId, reader, safePage, nowIso()).run();
  await env.DB.prepare("UPDATE books SET updated_at = ? WHERE id = ?").bind(nowIso(), bookId).run();
...
async function addComment(env: Env, request: Request): Promise<Response> {
  ...
  if (!book) return json({ error: "Book not found" }, { status: 404 });
  const safePage = Math.min(page, Math.max(1, book.total_pages));
  const comment = { id: newId("comment"), book_id: bookId, page: safePage, author, content, created_at: nowIso() };
  await env.DB.prepare(
...
let currentBook='',currentPage=1,totalPages=1,currentReader='layla';
...
async function loadBooks(){const data=await api('/books/api/list');const shelf=document.getElementById('shelf');if(!data.books.length){shelf.innerHTML='<div class="card empty">书架空空如也</div>';return;}shelf.innerHTML=data.books.map(book=>{const lp=Math.round((book.progress.layla/book.total_pages)*100);const kp=Math.round((book.progress.kld/book.total_pages)*100);return `<article class="book-item" onclick="openBook('${esc(book.id)}')"><div class="book-title">${esc(book.title)}</div><div class="book-meta">共 ${book.total_pages} 页</div><div class="progress-bar"><div class="progress-fill layla" style="width:${lp}%"></div></div><div class="progress-bar"><div class="progress-fill kld" style="width:${kp}%"></div></div><div class="progress-labels"><span>蕾拉 ${book.progress.layla}页</span><span>克 ${book.progress.kld}页</span></div></article>`}).join('');}
...
async function openBook(id){currentBook=id;const data=await api('/books/api/page?book_id='+encodeURIComponent(id)+'&page=1');currentPage=data.progress[currentReader]||1;totalPages=data.total_pages;document.getElementById('shelf').style.display='none';document.getElementById('reader').classList.add('active');await loadPage();}
...
async function loadPage(){const data=await api('/books/api/page?book_id='+encodeURIComponent(currentBook)+'&page='+currentPage);document.getElementById('readerTitle').textContent=data.title;document.getElementById('content').textContent=data.content;document.getElementById('currentPage').textContent=data.page;document.getElementById('totalPages').textContent=data.total_pages;currentPage=data.page;totalPages=data.total_pages;document.getElementById('prevBtn').disabled=currentPage<=1;document.getElementById('nextBtn').disabled=currentPage>=totalPages;renderComments(data.comments||[]);await api('/books/api/progress',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({book_id:currentBook,reader:currentReader,page:currentPage})});}
async function changePage(delta){const next=currentPage+delta;if(next<1||next>totalPages)return;currentPage=next;await loadPage();}
function renderComments(comments){const list=document.getElementById('commentsList');if(!comments.length){list.innerHTML='<div class="empty">这一页还没有批注</div>';return;}list.innerHTML=comments.map(c=>`<div class="comment"><div class="comment-author">${c.author==='kld'?'克':'蕾拉'}</div><div class="comment-content">${esc(c.content)}</div><div class="comment-time">${new Date(c.time).toLocaleString('zh-CN')}</div></div>`).join('');}
async function addComment(){const input=document.getElementById('commentInput');const content=input.value.trim();if(!content)return;await api('/books/api/comments',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({book_id:currentBook,page:currentPage,author:currentReader,content})});input.value='';await loadPage();}
loadBooks().catch(e=>{document.getElementById('shelf').innerHTML='<div class="card empty">加载失败：'+esc(e.message)+'</div>';});
