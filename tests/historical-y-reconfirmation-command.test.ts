...
describe("historical Y reconfirmation command", () => {
  ...
  it("requires the exact manifest id for apply and makes one bounded request", async () => {
    ...
    expect(report).toMatchObject({ batch_id: plan.batchId, selected: 1, remaining: 0 });
  });
